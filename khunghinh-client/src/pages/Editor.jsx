...
import useImage from 'use-image'   // npm i use-image (nếu chưa có)
...
function FrameOverlay({ url }) {
  // PNG nền trong suốt dùng làm khung
...
export default function Editor() {
  ...
  const [alias, setAlias] = useState(new URLSearchParams(location.search).get('alias') || 'tet')
  const [frame, setFrame] = useState(null)
  const [frames, setFrames] = useState([])
  const [thickness, setThickness] = useState(40)       // fallback khi không có PNG
  const [color, setColor] = useState('#2563eb')        // fallback khi không có PNG
  const [qrOpen, setQrOpen] = useState(false)
  const [crop, setCrop] = useState({ enabled: false, x: 100, y: 100, size: 600 })

  useEffect(() => { getFrames().then(setFrames) }, [])
  useEffect(() => {
    getFrameByAlias(alias).then(f => { setFrame(f); if (f?.color) setColor(f.color) })
  }, [alias])
  ...
  const exportPng = () => {
    ...
    let dataUrl
    if (crop.enabled) {
      st.width(crop.size); st.height(crop.size); st.x(-crop.x); st.y(-crop.y)
      st.draw()
      dataUrl = st.toDataURL({ mimeType: 'image/png', pixelRatio: 2 })
      // reset
      st.x(0); st.y(0); st.width(oldW); st.height(oldH); st.draw()
    } else {
      dataUrl = st.toDataURL({ mimeType: 'image/png', pixelRatio: 2 })
    }
    const a = document.createElement('a')
    a.href = dataUrl; a.download = `khung-${alias}.png`; a.click()
  }
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 py-12'>
      ...
      <div className='card p-4 mb-6 grid lg:grid-cols-[1fr_340px] gap-6'>
        ...
        <div className='border rounded-xl overflow-hidden relative'>
          <Stage ref={stageRef} width={800} height={800}>
            <Layer>
              ...
              {/* 👉 KHUNG PNG ONLINE (nếu có) – dùng frame.thumb */}
              {frame?.thumb ? (
                <FrameOverlay url={frame.thumb} />
              ) : (
                // Fallback: viền màu khi chưa có PNG
                <Rect
                  x={0}
                  y={0}
                  width={800}
        ...
        <div className='space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-3'>
            ...
            <label className='text-sm'>Độ dày khung {thickness}px</label>
            <input type='range' min='10' max='100' step='2' value={thickness} onChange={e => setThickness(+e.target.value)} />
          </div>
      ...
      <QrModal open={qrOpen} onClose={() => setQrOpen(false)} text={location.href} />
    </div>
  )
}
